...
// 🎵 Background Music
const bgMusic = new Audio('/assets/spookybackground.mp3');
...
function handleTrickOrTreat() {
    const chance = Math.random();

    if (chance < 0.7) { 
        // 🎁 70% chance: get 1–5 candies
        const candy = Math.floor(Math.random() * 5) + 1;
        candyScore += candy;
        socket.emit('trickResult', { username, candy });
        alert(`🍬 You got ${candy} candies! Total: ${candyScore}`);
    } else {
        // 😱 30% chance: lose 1–4 candies
        const loss = Math.floor(Math.random() * 4) + 1; // 1–4
        candyScore = Math.max(0, candyScore - loss);
        socket.emit('trickResult', { username, candy: -loss });

        // Little visual "jump scare" effect
        document.body.style.backgroundColor = '#ff0000';
        setTimeout(() => { document.body.style.backgroundColor = ''; }, 150);

        alert(`😱 Boo! You got tricked and lost ${loss} candy${loss > 1 ? 'ies' : 'y'}!`);
    }
}

// Intercept chat input for "/trick"
function sendChat() {
    const msg = chatInput.value.trim();
    if (!msg) return;
...
function draw() {
    ...
    // 🏆 Leaderboard update from server
