...
const mapBarriers = {
    main: [
        ...
        { x: 255, y: 170, width: 150, height: 150 }  // Bush barrier 
    ],
    // sled: [
    //     // Sled map barriers - adjust these coordinates to match your sled map layout
    //     { x: 0, y: 0, width: 1920, height: 220 }, // Top boundary
    //     { x: 0, y: 0, width: 100, height: 1080 }, // Left boundary
    //     { x: 1820, y: 0, width: 100, height: 1080 }, // Right boundary
    //     { x: 0, y: 980, width: 1920, height: 100 }, // Bottom boundary
    //     // Add more sled-specific barriers as needed
    //     { x: 1250, y: 150, width: 200, height: 200 }, // Example obstacle 1
    //     // { x: 1100, y: 120, width: 200, height: 50 }  // Example obstacle 2
    // ]
};
...
mapImages.main.src = 'assets/map1_winter.png';
// mapImages.sled.src = 'assets/sled_map.png';

// Current map state
let currentMap = 'main';
let otherPlayersVisible = true; // Tracks if other players should be visible
...
const speed = 3;
// Default spawn positions for each map
const spawnPositions = {
    main: { x: 100, y: 340 },
    // sled: { x: 200, y: 200 }  // Adjust these coordinates for the sled map
};

let localPlayer = { 
    x: spawnPositions.main.x, 
    y: spawnPositions.main.y, 
    direction: 'right', 
    moving: false, 
    chat: '' 
};
...
document.getElementById('joinBtn').onclick = () => {
    ...
    // Set initial position based on current map
    localPlayer.x = spawnPositions[currentMap].x;
    localPlayer.y = spawnPositions[currentMap].y;
...
function handleCommand(command) {
    ...
    switch(cmd) {
        case '/sled':
            ...
            // Update position to sled map spawn
            localPlayer.x = spawnPositions.sled.x;
            localPlayer.y = spawnPositions.sled.y;
            localPlayer.chat = 'Wheee! Sled time!';
            break;
            
        case '/home':
            currentMap = 'main';
            otherPlayersVisible = true;
            // Update position to main map spawn
            localPlayer.x = spawnPositions.main.x;
            localPlayer.y = spawnPositions.main.y;
            localPlayer.chat = 'Back home!';
            break;
            
        default:
            localPlayer.chat = 'Unknown command: ' + cmd;
    }
    
    // Show the command feedback for 3 seconds
    setTimeout(() => { localPlayer.chat = ''; }, 3000);
}
...
function update() {
    ...
    let collides = false;
    const currentBarriers = mapBarriers[currentMap] || [];
    for (const barrier of currentBarriers) {
        if (otterBox.x < barrier.x + barrier.width && 
            otterBox.x + otterBox.width > barrier.x && 
            otterBox.y < barrier.y + barrier.height && 
            otterBox.y + otterBox.height > barrier.y) { 
            collides = true; 
            break; 
        }
    }
...
function draw() {
    ...
    // Draw the current map if loaded
    if (mapImages[currentMap].complete) {
        ctx.drawImage(mapImages[currentMap], 0, 0, canvas.width, canvas.height);
    }

    // // Debug: Show barriers (red for main map, blue for sled map)
    // if (currentMap === 'main') {
    ...
    // Only draw other players if they should be visible
    for (let id in players) {
        // Skip other players if we're on the sled map
        if (id !== socket.id && !otherPlayersVisible) continue;
        const p = players[id];
        const anim = playerAnimations[id];
        const now = Date.now();
